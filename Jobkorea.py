...
def local_count(df):
    ...
    plt.savefig("D:/Code/Python/JobkoreaCrawler/Local_chart.jpg")
    plt.show()
    ...
    plt.imshow(cloud)
    plt.axis('off')
    cloud.to_file("D:/Code/Python/JobkoreaCrawler/Local_cloud.jpg")
    plt.show()
...
def cause_effect_graph(df):
    ...
    plt.savefig("D:/Code/Python/JobkoreaCrawler/pay_starscore_chart.jpg")
    plt.show()
    
def main():
    try:
        df = pd.read_csv('D:/Code/Python/JobkoreaCrawler/jobkorea.csv', encoding = 'cp949')
    except:
        result = []
        print('Crawling ...')
        crawl_companyinfo(result)
        tbl = pd.DataFrame(result, columns = ('평점순위', '회사명', '지역', '평점', '연봉'))
        tbl.to_csv('D:/Code/Python/JobkoreaCrawler/jobkorea.csv', encoding = 'cp949', mode = 'w', index = False)
        del result[:]
        df = pd.read_csv('D:/Code/Python/JobkoreaCrawler/jobkorea.csv', encoding = 'cp949')
